    ...
    return (
        ...
        >
            <nav
                ...
                style={{ backgroundColor: baseColor, border: '1px solid rgba(255,255,255,0.05)' }}
            >
                <div className="card-nav-top absolute inset-x-0 top-0 h-[60px] flex items-center justify-between p-2 pl-[1.1rem] z-[2]">
                    {/* Dark/Light mode adaptation for the hamburger lines */}
                    <div
                        ...
                        tabIndex={0}
                        style={{ color: menuColor || (theme === 'dark' ? '#fff' : '#000') }}
                    >
                        <div
                    ...
                    <div className="logo-container flex items-center md:absolute md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 order-1 md:order-none cursor-pointer">
                        {logo ? (
                            ...
                            <span className="font-bold text-xl tracking-tighter" style={{ color: menuColor || (theme === 'dark' ? '#fff' : '#000') }}>
                                B<span className="text-[#00FF94]">GB</span>
                            </span>
                        )}
                    </div>
                ...
                >
                    {(items || []).slice(0, 3).map((item, idx) => (
                        <div
                            key={`${item.label}-${idx}`}
                            className="nav-card select-none relative flex flex-col gap-2 p-[12px_16px] rounded-[calc(0.75rem-0.2rem)] min-w-0 flex-[1_1_auto] h-auto min-h-[60px] md:h-full md:min-h-0 md:flex-[1_1_0%] border shadow-md transition-transform hover:scale-[1.02]"
                            ref={setCardRef(idx)}
                            style={{ backgroundColor: item.bgColor, color: item.textColor, borderColor: 'rgba(255,255,255,0.05)' }}
                        >
                            <div className="nav-card-label font-bold tracking-tight text-[18px] md:text-[22px]">
                                {item.label}
                            </div>
